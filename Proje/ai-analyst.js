...
class AIAnalyst {
    ...
    /**
     * Öneri Sistemi için Parametre Analizi
     * Konuyu ve seviyeyi alıp, DB ile eşleşecek "keywords" ve "englishLevel" döndürür.
     */
    async getRecommendationParams(topic, userLevel) {
        console.log(`Öneri Parametreleri Analiz Ediliyor: ${topic} - ${userLevel}`);

        const prompt = `
        Kullanıcı Konusu: "${topic}"
        Kullanıcı Seviyesi: "${userLevel}"

        Görevin:
        Bu öğrenci için YouTube'da aratılacak EN İYİ anahtar kelime cümlesini (searchQuery) belirle.
        Örnekler:
        - Konu: "Türev", Seviye: "Başlangıç" -> "sıfırdan türev konu anlatımı"
        - Konu: "Photosynthesis", Seviye: "Advanced" -> "photosynthesis mechanism advanced lecture"
        - Konu: "Python", Seviye: "Orta" -> "python orta seviye projeler"

        Çıktı Formatı (Saf JSON):
        {
            "searchQuery": "YouTube arama terimi"
        }
        `;
        ...
        try {
            ...
            console.error("Recommendation params error:", err);
            // Fallback
            return {
                searchQuery: `${topic} ${userLevel} seviye konu anlatımı`
            };
        }
    }
}
